...
void UCLACH(cv::Mat& image, std::vector<cv::KeyPoint>& keypoints, uint64_t*& h_GPUdesc, std::vector<uint2>& kps) {
	// ------------- UCLATCH ------------
	...
	h_GPUdesc = new uint64_t[8 * kps.size()];
...
int main() {
	...
	UCLACH(image, keypoints, h_GPUdesc, kps);

	long long total = 0;
	for (size_t i = 0; i < 8 * kps.size(); ++i) 
		total += h_GPUdesc[i];
